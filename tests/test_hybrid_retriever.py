...
def test_init(mock_index, monkeypatch):  # noqa: ARG001
    ...
    retriever = UpstashHybridRetriever(
...
def test_to_dict_from_dict(mock_index, monkeypatch):  # noqa: ARG001
    ...
    assert data == {
        "type": "haystack_integrations.components.retrievers.upstash.hybrid_retriever.UpstashHybridRetriever",
        "init_parameters": {
            "document_store": {
    ...
    restored = UpstashHybridRetriever.from_dict(data)
...
def test_run(mock_index, monkeypatch):
    ...
    store = UpstashDocumentStore()
    retriever = UpstashHybridRetriever(document_store=store)

    sparse_emb = SparseEmbedding(indices=[0, 1], values=[0.5, 0.8])
    result = retriever.run(query_embedding=[0.1, 0.2, 0.3], query_sparse_embedding=sparse_emb, top_k=2)
    ...
    assert kwargs["sparse_vector"] == ([0, 1], [0.5, 0.8])
